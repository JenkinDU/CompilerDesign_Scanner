	...
	public Value getUseableValue(CodeGenerationContext c) throws SemanticException {
		...
		if (first instanceof NumberValue && second instanceof RegisterValue) {
			if (operator.commutative) {
				...
				c.appendInstruction(new ImmediateMathOperationInstruction(operator.immediateOpcode, temp, aReg,
						((NumberValue) first).intValue()));
		...
		if (first instanceof NumberValue && second instanceof NumberValue) {
			return new NumberValue(
					operator.operate(((NumberValue) first).intValue(), ((NumberValue) second).intValue()));
		}
		throw new SemanticException(
				"Unexpected combination of types for a and b: " + first.getClass() + ", " + second.getClass());
	}
