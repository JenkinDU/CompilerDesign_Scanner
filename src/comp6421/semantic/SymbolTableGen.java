...
public class SymbolTableGen implements ActionCallback {
	...
	// }
	// System.out.println("Create table:" + action.getClass().getName() + ",
	// Token:"+ t);
	// }
	//
	// @Override
	...
	// error+="Line "+t.getPosition() + ", " +e.getMessage()+"\n";
	// }
	// System.out.println("\t"+"Insert entry:" + action.getClass().getName() +
	// ", Token:"+ t);
	// }
	...
	public void createTable(String action, Token p, Token c) {
		if (("sem_PushVariableName".equals(preAction) || "sem_EndAdditionExpression".equals(preAction))
				&& "sem_PushVariableName".equals(action) && c.getTYPE() == EType.OPENPAR) {
			return;
		}
			...
			if (a != null) {
				if (TableContext.getInstance().showMigration && token.length() == 0) {
					if ("sem_StartRelationExpression".equals(action) && p.getTYPE() == EType.ASSGN) {
						...
						System.out.println(
								String.format("%-70s", String.format("%-10s", "action") + "\t" + "stack") + "expr\n");
					}
					if (recordMigration) {
				...
				if (TableContext.getInstance().showMigration) {
					...
					if (recordMigration) {
						String stack = action.substring(4, action.length() < 14 ? action.length() : 14) + "\t"
								+ MigrationContext.instance.getStackString();
						System.out.println(String.format("%-70s", stack) + "" + token);
					}
				}
			}
