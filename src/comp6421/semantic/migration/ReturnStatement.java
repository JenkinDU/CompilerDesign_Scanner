...
public class ReturnStatement extends Expression implements Statement {
	...
	public void generateCode(CodeGenerationContext c) throws SemanticException {
		...
		if (useableReturnVal instanceof RegisterValue) {
			c.appendInstruction(new AddWordInstruction(Register.RETURN_VALUE, Register.ZERO,
					((RegisterValue) useableReturnVal).getRegister()));
		} else if (useableReturnVal instanceof NumberValue) {
			c.appendInstruction(new AddWordImmediateInstruction(Register.RETURN_VALUE, Register.ZERO,
					((NumberValue) useableReturnVal).intValue()));
		}
	}
